# 🛍️ TopShop Bot — TopShop Pakistan AI Chat Assistant

An AI-powered sales chatbot embedded on the **TopShop Pakistan** Shopify store
(men.s makeup, skincare, haircare, beauty devices women.s fashion, tees, shirts, jeans, packs). It chats with customers, searches live
...
- **Order tracking** — by order number (#TS), tracking ID, phone, or email. Shows order name,
...
- **Place an order** — a form that sends the order to your WhatsApp (has a Size field (clothing)).
...
- **Product search** — Shopify `suggest.json` (the store's own search engine) ranks results;
  ...
  - "all products" / "fashion" / "everything" → a catalog sample.
  - Typos are cleaned by the AI first ("foundaton" → "foundation").
- **Discount** — "sale / discount / deal" → discounted items, highest % first.

> Search relies on Shopify's storefront engine + live Admin data. (A fashion synonym map
> can be added later from the TopShop product CSV for extra typo/synonym smarts.)
...
| `SHOPIFY_STORE` | `wxc0s1-a7.myshopify.com` |
| `SHOPIFY_ADMIN_TOKEN` | Shopify Admin API access token (`shpat_…`) |
| `WHATSAPP_NUMBER` | `923079542935` |
| `PUBLIC_DOMAIN` | `https://topshoppk.com` |
| `ALLOWED_ORIGIN` | `https://topshoppk.com` (use `*` while testing) |
...
2. **Widget:** in Shopify → Online Store → Themes → Edit code → Snippets → add
   ...
   ```js
   var VERCEL_URL = "https://topshop-chatbot.vercel.app/api/chat";
   ```
...
**Health check:** open `https://topshop-chatbot.vercel.app/api/chat`
...
- Theme: navy + red + white; stylish T logo; **Live products** indicator dot in the header.
- Order prefix: **#TS**. Carriers: **PostEx** and **OwnExpress**.
