// api/chat.js — Trenzee "TopShop Bot" backend
//
...
const SYNONYMS = {
  // fashion synonyms (suggest.json does most of the work; expand later from CSV)
  tee: ["shirt", "tee", "tshirt", "t-shirt"],
  tshirt: ["shirt", "tee", "tshirt", "t-shirt"],
  tshirts: ["shirt", "tee", "tshirt"],
  shirt: ["shirt", "tee"],
  jeans: ["jeans", "denim", "trouser", "pant"],
  jean: ["jeans", "denim", "trouser"],
  pant: ["trouser", "pant", "jeans"],
  pants: ["trouser", "pant", "jeans"],
...
const PRODUCT_NOUNS = new Set([
  "shirt", "shirts", "tee", "tshirt", "polo", "top", "blouse", "jeans", "denim", "trouser",
  "trousers", "pant", "pants", "shorts", "short", "hoodie", "sweatshirt", "jacket", "coat",
  "dress", "frock", "kurti", "kurta", "abaya", "skirt", "cap", "hat", "socks", "suit", "outfit",
  "pack", "men", "women", "boys", "girls",
])
...
// Broad (e.g. "shirt", "jeans") -> more, for browsing.
function trimBySpecificity(query, results) {
  ...
  return results.slice(0, 10);               // short/browse query ("shirt", "jeans") -> more
}
...
async function shopifySuggest(query, catalog) {
  const domain = (process.env.PUBLIC_DOMAIN || "https://topshoppk.com").replace(/\/+$/, "");
  const q = (query || "").trim();
  if (!q) return null;
...
async function findOrderByNumber(orderId) {
  ...
  const candidates = [digits, `TS${digits}`, `#TS${digits}`, `#${digits}`, String(orderId)];
...
async function detectIntent(messages) {
  ...
  const NOUNS = "shirt, tee, t-shirt, polo, top, blouse, jeans, denim, trouser, pant, shorts, hoodie, sweatshirt, jacket, coat, dress, frock, kurti, kurta, abaya, skirt, cap, pack of 3, men, women, boys, girls";
  const prompt = `You are the intent classifier + search normalizer for TopShop Pakistan, a men's & women's fashion store in Pakistan.
...
- Fix spelling and phonetic errors and map to the closest store word. Examples: "tshrt"->"tshirt", "trowser"->"trouser", "hoody"->"hoodie", "jaket"->"jacket".
- Map synonyms to store words: "jeans"->"trouser denim", "tee"->"shirt tshirt", "frock"->"dress".
  ...
  try {
    ...
    if (body.track) {
      ...
      const msg = {
        ...
        tracking_not_found: "I couldn't match that tracking number to an order. Please enter your Order number (e.g. #TS1234) instead, or message us on WhatsApp.",
    ...
    if (intentData.intent === "other" &&
        /\b(product|products|clothes|clothing|fashion|shirt|tee|jeans|trouser|hoodie|dress|men|women|show|dikhao|chahiye|chahie|all products|everything)\b/i.test(lastMsg)) {
    ...
    if (intentData.intent === "product") {
      ...
      if (/\b(discount|discounted|sale|deal|deals|offer|offers|cheap|off)\b/.test(q)) {
        ...
        // Broad browse ("all products", "everything", "clothes") -> sample of the catalog
        if ((!results || !results.length) && /\b(all|every|everything|sab|saray|sary|clothes|clothing|fashion|men|women|products?|items?)\b/.test(q)) {
    ...
    const fallback = products.length
      ...
      : (action === "agent" ? "You can chat with our team on WhatsApp using the button below." : "How can I help you find the perfect outfit?");
    ...
    await logChat({
      store: "TopShop Bot", session: body.session || "",
